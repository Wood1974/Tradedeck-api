...
import config
from auth import (
    draw_payee_id, get_draw, get_draw_schedule, get_escrow_for_draw,
    get_job, is_job_owner, is_schedule_owner, require_auth,
...
def _escrow_error_response(exc):
...
def _safe_storage_path(draw_id, requested_path=None):
    ...
    if requested_path:
        ...
        if cleaned.startswith("draws/") and draw_id in cleaned: return cleaned
    return f"draws/{draw_id}/{utc_now_iso().replace(':','-')}.jpg"
...
def _analyze_photo(image_bytes, media_type):
    ...
    prompt = ('You are a construction quality inspector reviewing a milestone photo for payment release. '
...
def escrow_create():
    data = request.get_json(silent=True) or {}
    missing = [f for f in ["job_id","draw_id","amount_cents"] if data.get(f) in (None,"")]
    if missing: return _error("Missing: " + ", ".join(missing))
    try:
        result = create_escrow_payment(supabase_admin, dict(data), g.user_id)
        return jsonify(result)
    except EscrowError as exc: return _escrow_error_response(exc)
    except Exception:
...
def escrow_release():
    ...
    if not draw_id: return _error("draw_id required")
    draw = get_draw(supabase_admin, draw_id)
    if not draw: return _error("Draw not found", 404)
    schedule = get_draw_schedule(supabase_admin, draw.get("job_id"))
    if not is_schedule_owner(schedule, g.user_id): return _error("Only the job owner can release escrow", 403)
    try:
...
def escrow_refund():
    ...
    if not is_schedule_owner(schedule, g.user_id) and not (escrow and escrow.get("payer_id") == g.user_id):
...
def get_draw_detail(draw_id):
    try:
        photos = supabase_admin.table("draw_photos").select("*").eq("draw_id",draw_id).execute()
        return jsonify({"draw":g.draw,"escrow":g.escrow,"photos":photos.data})
    except Exception:
...
def approve_draw(draw_id):
    draw = g.draw; escrow = g.escrow
    if draw.get("status") != "submitted": return _error("Draw must be submitted before approval", 409)
    if not escrow: return _error("No escrow record for this draw", 404)
    if escrow.get("status") != "held": return _error("Escrow must be funded and held", 409)
    try:
        supabase_admin.table("draws").update({"status":"approved","approved_at":utc_now_iso()}).eq("id",draw_id).execute()
        transfer_id = release_escrow(supabase_admin, draw_id)
        return jsonify({"success":True,"transfer_id":transfer_id})
    except EscrowError as exc: return _escrow_error_response(exc)
    except Exception:
...
def upload_photo(draw_id):
    ...
    if not image_b64: return _error("image_base64 required")
    if g.draw.get("status") in ("approved","released"): return _error("Cannot upload photos for a closed draw", 409)
    try:
        ...
        supabase_admin.storage.from_(DRAW_PHOTOS_BUCKET).upload(storage_path, image_bytes, {"content-type":media_type,"upsert":"false"})
        ai = _analyze_photo(image_bytes, media_type)
        score = int(ai.get("score",0)); passed = bool(ai.get("passed",False)); flags = ai.get("flags",[])
        photo_result = supabase_admin.table("draw_photos").insert({"draw_id":draw_id,"uploaded_by":g.user_id,"storage_path":storage_path,"ai_analysis":ai,"ai_passed":passed,"ai_score":score,"ai_summary":ai.get("summary",""),"ai_flags":flags}).execute()
        if g.draw.get("status") not in ("approved","released"):
            supabase_admin.table("draws").update({"status":"submitted","submitted_at":utc_now_iso()}).eq("id",draw_id).execute()
        return jsonify({"success":True,"photo_id":photo_result.data[0]["id"],"storage_path":storage_path,"ai_score":score,"ai_passed":passed,"ai_summary":ai.get("summary",""),"ai_flags":flags,"ai_recommendations":ai.get("recommendations",[])})
    except json.JSONDecodeError: return _error("AI returned invalid JSON", 500)
...
def stripe_webhook():
    ...
    obj = event.get("data",{}).get("object",{}); metadata = obj.get("metadata") or {}; draw_id = metadata.get("draw_id")
    try:
        if event_type == "payment_intent.amount_capturable_updated" and draw_id: mark_escrow_held(supabase_admin, draw_id)
        elif event_type == "payment_intent.payment_failed" and draw_id: supabase_admin.table("stripe_escrow").update({"status":"failed"}).eq("draw_id",draw_id).eq("status","pending").execute()
        elif event_type == "payment_intent.canceled" and draw_id: supabase_admin.table("stripe_escrow").update({"status":"refunded"}).eq("draw_id",draw_id).in_("status",["pending","held"]).execute()
