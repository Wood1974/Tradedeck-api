...
def escrow_release():
    ...
    job = get_job(supabase_admin, draw.get("job_id"))
...
def escrow_refund():
    ...
    if not is_job_owner(job, g.user_id) and not (
...
def approve_draw(draw_id):
    draw = g.draw
    escrow = g.escrow
    if draw.get("status") not in ("ai_approved", "owner_approved"):
        return _error("Draw must pass AI review before owner approval", 409)
    if not escrow:
    ...
    try:
        supabase_admin.table("draws").update(
            {"status": "owner_approved", "approved_at": utc_now_iso()}
...
def upload_photo(draw_id):
    ...
    if g.draw.get("status") in ("released", "refunded", "disputed"):
    ...
    try:
        ...
        if g.draw.get("status") not in ("owner_approved", "released"):
            supabase_admin.table("draws").update(
                {
