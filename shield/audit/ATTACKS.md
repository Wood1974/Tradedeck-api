...
| 3 | Stamp a stock photo with the site's coordinates using `piexif` | **Works today.** Returned `gps_corroborated: True`, distance 0.1 m, no integrity note. Twelve lines, using the same library Shield reads EXIF with. **Not closed** — see AR-1. Mitigated by a buyer-set geofence and solar geometry; only capture-time attestation closes it. |
